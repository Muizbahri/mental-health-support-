...
export default function CounselorSignUpPage() {
  ...
  const [mapKey, setMapKey] = useState(0); // force rerender
  ...
  function handleICChange(e) {
    ...
    let age = "";
    // Only calculate age if IC has at least 6 digits for date parsing
    if (/^\d{6}/.test(ic)) {
      const now = new Date();
  ...
  async function handleLocationSearch() {
    ...
    if (geo) {
      setForm(f => ({ ...f, latitude: geo.lat, longitude: geo.lon, location: geo.formatted }));
  ...
  async function handleSubmit(e) {
    ...
    if (!form.icNumber || form.icNumber.length !== 12 || !/^\d{12}$/.test(form.icNumber)) {
      alert('IC Number must be exactly 12 digits with no dashes or non-numeric characters.');
      return;
    }
    
    // Validate required fields
    const requiredFields = [
      "fullName", "email", "icNumber", "age", "phoneNumber", "registrationNumber", "certificate", "profileImage", "location", "address", "latitude", "longitude", "password"
    ];
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-50 to-pink-50 relative">
      ...
      <div className="flex flex-col items-center justify-center w-full">
        <div className="bg-white rounded-3xl shadow-xl px-8 py-10 w-full max-w-md mt-24">
          <h1 className="text-4xl font-bold mb-6 text-center text-gray-800">Counselor Sign Up</h1>
          <form className="space-y-4" onSubmit={handleSubmit} autoComplete="off">
            ...
            <div>
              <label className="block text-lg font-semibold text-gray-700 mb-1">Registration Number</label>
              <div className="relative">
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                ...
                <button type="button" className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg transition" onClick={async () => {
                  if (!form.address) return;
                  const geo = await geocodeAddress(form.address);
                  if (geo) {
                    setForm(f => ({ ...f, latitude: geo.lat, longitude: geo.lon, address: geo.formatted }));
                    setMapPos({ lat: geo.lat, lon: geo.lon });
                    setMarkerPos({ lat: geo.lat, lon: geo.lon });
                    setMapKey(k => k + 1);
                  }
            ...
            <div className="flex gap-4">
              <div className="w-1/2">
                ...
                <input
                  ...
                  className="w-full pl-4 pr-4 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-200 text-gray-700 bg-gray-50"
                  value={form.latitude}
