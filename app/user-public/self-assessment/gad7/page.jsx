...
export default function GAD7Assessment() {
  ...
  const handleSubmit = () => {
    ...
    router.push(`/user-public/self-assessment/gad7/result?answers=${encodeURIComponent(answersStr)}`);
  };

  const progress = ((currentQuestionIndex + 1) / gad7Questions.length) * 100;
  ...
  return (
    <div className="min-h-screen flex bg-gray-50 relative">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-10 max-w-2xl mx-auto w-full transition-all duration-200">
        <h1 className="text-2xl sm:text-3xl font-bold text-center text-gray-900 mb-4">GAD-7 Self-Assessment</h1>
        <div className="bg-white rounded-xl shadow p-4 sm:p-8 flex flex-col gap-6 w-full">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-gray-500 font-medium">Question {currentQuestionIndex + 1} of {gad7Questions.length}</span>
          </div>
          {/* Progress bar */}
          ...
          <div className="bg-white rounded-2xl shadow p-8 mb-8">
            <h2 className="text-xl font-bold text-black text-center mb-2">GAD-7 Anxiety Assessment</h2>
            ...
            <div className="font-semibold mb-4 text-gray-800">{gad7Questions[currentQuestionIndex]}</div>
